...
export function validateSpringConfig(
  ...
  if (config.stiffness !== undefined) {
    if (config.stiffness < SPRING_RANGES.stiffnessMin) {
      errors.push(
        `Stiffness ${config.stiffness} below minimum ${SPRING_RANGES.stiffnessMin}`
      )
    }
    if (config.stiffness > SPRING_RANGES.stiffnessMax) {
      errors.push(
        `Stiffness ${config.stiffness} above maximum ${SPRING_RANGES.stiffnessMax}`
      )
    }
  }
  ...
  if (config.damping !== undefined) {
    if (config.damping < SPRING_RANGES.dampingMin) {
      errors.push(
        `Damping ${config.damping} below minimum ${SPRING_RANGES.dampingMin}`
      )
    }
    if (config.damping > SPRING_RANGES.dampingMax) {
      errors.push(
        `Damping ${config.damping} above maximum ${SPRING_RANGES.dampingMax}`
      )
    }
  }
  ...
  if (config.mass !== undefined && config.mass !== SPRING_RANGES.mass) {
    errors.push(
      `Mass ${config.mass} must be ${SPRING_RANGES.mass}`
    )
  }
