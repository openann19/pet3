...
export class LiveStreamingAPI {
  ...
  async createRoom(
    ...
    // TODO: Sign tokens server-side using LiveKit SDK
    // For now, return placeholder tokens
    const joinToken = `join_token_${generateULID()}`
    const publishToken = `publish_token_${generateULID()}`

    // TODO: Create LiveKit room via server-side API
  ...
  async joinStream(
    ...
    if (streamIndex !== -1) {
      const streamToUpdate = streams[streamIndex]
      if (streamToUpdate) {
