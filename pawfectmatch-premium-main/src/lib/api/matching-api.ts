import type { PetProfile, OwnerPreferences, SwipeRecord, MatchRecord } from '../domain/pet-model'
import type { MatchScore, HardGateResult } from '../domain/matching-engine'
import type { MatchingConfig } from '../domain/matching-config'
import type { Report, ReportReason } from '../contracts'
import { evaluateHardGates, calculateMatchScore } from '../domain/matching-engine'
import { DEFAULT_MATCHING_WEIGHTS, DEFAULT_HARD_GATES } from '../domain/matching-config'
...
export class MatchingAPI {
  ...
  async updatePreferences(prefs: OwnerPreferences): Promise<void> {
    prefs.updatedAt = new Date().toISOString()
    await window.spark.kv.set(`prefs:${prefs.ownerId}`, prefs)
  }
  ...
  async updateConfig(config: MatchingConfig): Promise<void> {
    config.updatedAt = new Date().toISOString()
    await window.spark.kv.set('matching-config', config)
